...
Третий камень вылез позже, уже на чужом устройстве: цикл прокрутки не проверял, что
страница двигается, а после цикла нажимал в любом случае. Когда жест не срабатывал,
пятнадцать холостых прокруток заканчивались тапом по ссылке, оставшейся под панелью, —
то есть тапом по кнопке «Language». Открывался диалог языков, статья оставалась
на экране, и падала уже следующая проверка:

```
Element should not exist {AppiumBy.id: org.wikipedia:id/page_contents_container}
Actual value: exists
```

По такому сообщению причину не найти: оно не о ссылке и не о панели. Теперь цикл
сверяет положение ссылки до и после жеста и не нажимает, если подвести её не удалось:

```
The article did not move, so the 'Java Software, Oracle' link stays under the action bar
(link top 1450, action bar top 1400). A tap there would go to the action bar, not to the link.
```

И отдельно — сам тап. По ссылке, уводящей из приложения, он иногда проходит впустую:
событие до страницы доходит, а перехода не начинается. Поэтому нажатие повторяется
до трёх раз, а если приложение так и не ушло со статьи, сообщение называет, что
на экране вместо браузера — это же сообщение отличает случай, когда на устройстве
нечем открыть внешнюю ссылку.
