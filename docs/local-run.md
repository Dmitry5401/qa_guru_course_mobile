...
ещё `Java (programming language)`, `JavaScript` и `JavaServer Pages`, а `findBy(text("Java"))`
берёт первое вхождение подстроки, то есть может открыть не ту статью — и тогда сверять заголовок
не с чем. Нужен `exactText`. И само название стоит держать одной константой: когда оно вписано
в поиск, в клик и в проверку по отдельности, правка легко применяется не везде.

Одного названия, впрочем, мало: «Java» называются и статья об острове, и статья о языке,
и статья о платформе, а после перехода по ссылке в шапке название нужной статьи есть
и на прежнем экране — в самой ссылке. Различает их подзаголовок: у него есть свой
идентификатор, и он не из WebView:
