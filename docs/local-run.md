...
облачному устройству язык задаётся явно, русский, чтобы `ArticleTests` читал ту же статью,
что и на телефоне (см. [stands.md](stands.md)).
...
ещё `JavaScript`, `JavaServer Pages` и `Java SE`, а проверка по вхождению подстроки берёт первое
совпадение, то есть может открыть не ту статью — и тогда сверять заголовок не с чем. Поэтому
заголовок первого результата сверяется через `exactText` до нажатия:
...
И само название стоит держать одной константой: когда оно вписано в поиск, в клик
и в проверку по отдельности, правка легко применяется не везде.

Одного названия, впрочем, мало: «Java» называются и статья о языке, и статья о платформе,
и статья об острове. Различает их подзаголовок — у него есть свой идентификатор, и он не
из WebView:
...
`ArticleTests` уходит из статьи по внешней ссылке. Первая попытка нажимала на текст ссылки:

```java
$(xpath("//android.widget.TextView[@text='Официальный сайт Java']")).click();
```
...
```
android.widget.TextView  clickable=false  text="Официальный сайт Java"  bounds=[93,2046][534,2103]
...
Панель «Сохранить / Язык / Найти в статье / Тема оформления / Содержание» висит поверх
статьи, а не под ней. Ссылка «Официальный сайт Java» после перехода в раздел оказалась
ровно под этой панелью:
...
Центр ссылки — `(333, 2074)`, а там уже кнопка «Язык». Тест открывал список языков
